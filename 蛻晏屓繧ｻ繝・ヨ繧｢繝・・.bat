@echo off
rem === Double-click this file (Windows) to run the first-time setup. ===
rem === Mac users: double-click "初回セットアップ.command" instead. ===
chcp 65001 >nul 2>&1
cd /d "%~dp0"
call :find_bash
...
echo  (Or: open this folder in "Git Bash" and run:  bash setup.sh )
echo.
pause
exit /b 1
